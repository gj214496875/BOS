...
<script type="text/javascript">
	...
	$(function() {
		...
		window.setTimeout(function(){
			$.messager.show({
				title:"消息提示",
				msg:"欢迎${loginUser.username}超级管理员！ <a href='javascript:void' onclick='top.showAbout();'>联系管理员</a>",
				timeout:5000
			});
		},3000);
		/*************/
        ...
        $("#btnEp").click(function () {
            //进行表单校验
            var b = $("#editPassWordWindow").form("validate")
            if(b){
                var password1 = $("#txtNewPass").val();
                var password2 = $("#txtRePass").val();
                if(password1==password2){
                    $.post(
                        "${pageContext.request.contextPath}/userAction_editPassWord.action",
                        {"password":password1},
                        function (data) {
                            if(data=="1"){
                                $.messager.alert("","修改成功","",function () {
                                    $("#editPwdWindow").window("close");
                                })

                            }else {
                                $.messager.alert("错误","修改失败，请重新输入","error")
                            }
                        },
                        "text"
                    )
                    ...
                    //两次输入的密码不一致，请重新输入
                    $.messager.alert("错误","两次输入的密码不一致，请重新输入","error")
                }
            }
        });
	});
	...
	function logoutFun() {
		...
		.confirm('系统提示','您确定要退出本次登录吗?',function(isConfirm) {
			if (isConfirm) {
				location.href = '${pageContext.request.contextPath }/userAction_exit';
			}
		});
	}
	// 修改密码
